...
namespace MyScholarshipForm {
	...
	public:
		Form1(void)
		{
			...
			f = new vector<Football>;
			iArrayUsed = new int(0);		// scholarship array is used initially
		}
	...
	private: System::Windows::Forms::Button^  bnAddress;
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->bnAddress = (gcnew System::Windows::Forms::Button());
			this->comboBoxType = (gcnew System::Windows::Forms::ComboBox());
			this->btnOpenTable = (gcnew System::Windows::Forms::Button());
			this->SuspendLayout();
			// 
			// label1
			// 
			...
			this->textBoxFirstName->Size = System::Drawing::Size(222, 26);
			...
			// bnAddress
			// 
			this->bnAddress->Location = System::Drawing::Point(364, 309);
			this->bnAddress->Margin = System::Windows::Forms::Padding(2);
			this->bnAddress->Name = L"bnAddress";
			this->bnAddress->Size = System::Drawing::Size(61, 24);
			this->bnAddress->TabIndex = 27;
			this->bnAddress->Text = L"Address";
			this->bnAddress->UseVisualStyleBackColor = true;
			this->bnAddress->Click += gcnew System::EventHandler(this, &Form1::bnAddress_Click);
			// 
			// comboBoxType
			// 
			this->comboBoxType->FormattingEnabled = true;
			this->comboBoxType->Items->AddRange(gcnew cli::array< System::Object^  >(2) {L"All", L"Football"});
			this->comboBoxType->Location = System::Drawing::Point(304, 50);
			this->comboBoxType->Name = L"comboBoxType";
			this->comboBoxType->Size = System::Drawing::Size(121, 21);
			this->comboBoxType->TabIndex = 30;
			this->comboBoxType->SelectedIndexChanged += gcnew System::EventHandler(this, &Form1::comboBoxType_SelectedIndexChanged);
			// 
			// btnOpenTable
			// 
			this->btnOpenTable->Location = System::Drawing::Point(144, 529);
			this->btnOpenTable->Name = L"btnOpenTable";
			this->btnOpenTable->Size = System::Drawing::Size(75, 23);
			this->btnOpenTable->TabIndex = 31;
			this->btnOpenTable->Text = L"Open Table";
			this->btnOpenTable->UseVisualStyleBackColor = true;
			this->btnOpenTable->Click += gcnew System::EventHandler(this, &Form1::btnOpenTable_Click);
			// 
			// Form1
			// 
			this->AutoScaleDimensions = System::Drawing::SizeF(6, 13);
			this->AutoScaleMode = System::Windows::Forms::AutoScaleMode::Font;
			this->ClientSize = System::Drawing::Size(465, 562);
			this->Controls->Add(this->btnOpenTable);
			this->Controls->Add(this->comboBoxType);
			this->Controls->Add(this->bnAddress);
...
#pragma endregion
	private: System::Void Form1_Load(System::Object^  sender, System::EventArgs^  e) {
			...
			ofstream fout;
			ofstream foutT;
			OpenFiles(fin, fout, finA, finF,foutT);

			while(!fin.eof())
				// Read a line from the file and push onto end ofscholars object array
				(*s).push_back(readFile(sFileLine, sParsedLine, fin));

			int sSize = s->size();  // get the size of the scholarship array
			// search for and add the right address
			for(int i = 0; i < sSize; i++)
			{
				while(!finA.eof())
				{
					Address a = readFile(sFileLine, sParsedLine, finA,(*s)[i].getID());
					if(a.getStreet() != "")  // did readFile find the record?
					{
						(*s)[i].stuAddress = a;
				...
				finA.clear(); // if not found, reset the EOF flag
				finA.seekg(0L, ios::beg); // reposition read cursor at beginning of file
			}
			while(!finF.eof())
			{
				(*f).push_back(readFile(sFileLine, sParsedLine, finF,'f'));
				// Fill the rest of the array from the other array
				// search for and add the right Scholarship info
				int j = (*f).size()-1; // get the current football index
				for(int i = 0; i < sSize; i++)
				{
					if((*f)[j].getID() == (*s)[i].getID())
					{
						(*f)[j].setAmount((*s)[i].getAmount());
						(*f)[j].setType((*s)[i].getType());
						(*f)[j].setLength((*s)[i].getLength());
						(*f)[j].setDate((*s)[i].getDate());
						(*f)[j].setLname((*s)[i].getLname());
						(*f)[j].setFname((*s)[i].getFname());
						(*f)[j].stuAddress = (*s)[i].stuAddress;
					}
				}
			}
			fillFormFields(0);
			comboBoxType->SelectedIndex = *iArrayUsed;  // Show "all" selected
		}
		...
		int fillFormFields(int p)
		{
			...
			labelNumS->Text = gcnew String(((*s)[i].getNumScholarships()-(*f)[0].getNumFs()).ToString());

			return i;  // return a copy of the currently displayed record's index
		}
		int fillFormFields(int p, char c)   // Overloaded fill for the football array
		{
			static int i = 0;  // Keeps track of which record we are on
			i += p;		       // changes the current record
			...
			if(i > int((*f).size()-1))  // Is the index within bounds
			{
				i = int((*f).size()-1); // reset to end of array
				if(p == 1)
					MessageBox::Show("Cannot go past end of array. Showing last record ",
									 "Notification", MessageBoxButtons::OK,MessageBoxIcon::Asterisk);
			}
			textBoxAmount->Text = gcnew String(addCommas((*f)[i].getAmount()).c_str());
			textBoxID->Text = gcnew String((*f)[i].getID().c_str());
			textBoxType->Text = gcnew String((*f)[i].getType().c_str());
			textBoxLength->Text = gcnew String((*f)[i].getLength().c_str());
			textBoxStarts->Text = gcnew String((*f)[i].getDate().c_str());
			textBoxLastName->Text = gcnew String((*f)[i].getLname().c_str());
			textBoxFirstName->Text = gcnew String((*f)[i].getFname().c_str());
			labelNumS->Text = gcnew String((*f)[i].getNumFs().ToString());
...
private: System::Void buttonPrevious_Click(System::Object^  sender, System::EventArgs^  e) {
			...
			if(*iArrayUsed == 0)
				fillFormFields(-1); // move the s array
			else
				fillFormFields(-1, 'f'); // move the f array
		 }
...
private: System::Void buttonFirst_Click(System::Object^  sender, System::EventArgs^  e) {
			...
			updateData();  // update the current form before moving to new record
			if(*iArrayUsed == 0)
				fillFormFields(-int(s->size()+1)); // move the s array to beginning
			else
				fillFormFields(-int(f->size()+1), 'f'); // move the f array to beginning
		 }
...
private: System::Void buttonLast_Click(System::Object^  sender, System::EventArgs^  e) {
			...
			updateData();  // update the current form before moving to new record
			if(*iArrayUsed == 0)
				fillFormFields(s->size()+1); // move the s array to end
			else
				fillFormFields((f->size()+1),'f'); // move the f array to end
...
private: System::Void btnSortByLastName_Click(System::Object^  sender, System::EventArgs^  e) {
			...
			delete temp;
			fillFormFields(-int(s->size()+1)); // set to beginning
		}
...
private: System::Void bnNew_Click(System::Object^  sender, System::EventArgs^  e) {
		 ...
		 void updateData()
		 {
			...
			if(*iArrayUsed == 0) // if using the Scholarship array
			{
			...
			else // update the football array
			{
				// get index of current record
				int i = fillFormFields(0,'f');   // This wipes out the data you just entered
				// Update the current record in the array of objects
				(*f)[i].setID(pID);
				(*f)[i].setType(pType);
				(*f)[i].setLength(pLength);
				(*f)[i].setDate(pDate);
				(*f)[i].setLname(pLname);
				(*f)[i].setFname(pFname);
				(*f)[i].setAmount(atoi(sAmount.data()));
			}
		 }
		 
private: System::Void bnAddress_Click(System::Object^  sender, System::EventArgs^  e) {
			 // Show the student's address in a messagebox
			 string sMessage = (*s)[fillFormFields(0)].stuAddress.getStreet() + "\n"
				             + (*s)[fillFormFields(0)].stuAddress.getCity() + ", "
							 + (*s)[fillFormFields(0)].stuAddress.getState() + ", "
							 + (*s)[fillFormFields(0)].stuAddress.getZip();
			 MessageBox::Show(gcnew String(sMessage.c_str()),
				              "Address", MessageBoxButtons::OK,
							   MessageBoxIcon::Information);
		 }

private: System::Void comboBoxType_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e) {

			 // Get the index of the selected item
			 int i = comboBoxType->SelectedIndex;
			 // Set which array to use for Form based on selection
			 // 0 = Scholarship Object array
			 // 1 = Football Objects array
			 switch(i)
			 {
			 case 0:	updateData();  // update the current form before moving to new record
						*iArrayUsed = 0;  // set currently used array to scholarship
						fillFormFields(-int(s->size()+1));   // Set Form to beginning of s array
						textBoxAmount->ReadOnly = false;
						textBoxID->ReadOnly = false;
						textBoxType->ReadOnly = false;
						textBoxStarts->ReadOnly = false;
						textBoxLastName->ReadOnly = false;
						textBoxFirstName->ReadOnly = false;
						bnAddAmount->Enabled = true;
						bnSubtractAmount->Enabled = true;

						break;

			 case 1:	updateData();  // update the current form before moving to new record
						*iArrayUsed = 1; // set currently used array to football
						fillFormFields(-int(f->size()+1),'f');   // Set Form to beginning of f array
						break;

			 default:	break;  // do nothing if invalid
			 }
		 }
private: System::Void btnOpenTable_Click(System::Object^  sender, System::EventArgs^  e) {
			 writeTable(*s);
			 FormTable^ myTable = gcnew FormTable();
			 myTable->ShowDialog();
		 }
};
}
