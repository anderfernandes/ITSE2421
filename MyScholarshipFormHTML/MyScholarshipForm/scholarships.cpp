...
void OpenFiles(ifstream &in, 
			   ...
			   ifstream &inA, 
			   ifstream &inF,
			   ofstream &outT)  // must be passed in by reference
{
...
void writeTable(vector<Scholarship> &sArray)
{
	...
	string sHeaders[] = {"ID", "Amount", "Type", "Length", 
		"Stars", "First Name", "Last Name", ""};
	...
	{
		string row[] = {sArray[i].getID(),
								addCommas(sArray[i].getAmount()),
										sArray[i].getType(),
										sArray[i].getLength(),
										sArray[i].getDate(),
										sArray[i].getFname(),
	...
	ofstream fout("c:\\temp\\ScholarshipsTable.html");
	if(!fout)
	{
