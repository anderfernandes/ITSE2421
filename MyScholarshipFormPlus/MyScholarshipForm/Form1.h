...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->comboBoxType->Location = System::Drawing::Point(1, 43);
...
#pragma endregion
	private: System::Void Form1_Load(System::Object^  sender, System::EventArgs^  e) {
			...
			fillFormFields(0);
			comboBoxType->SelectedIndex = *iArrayUsed;
		}
		...
		int fillFormFields(int p)
		{
			...
			labelNumS->Text = gcnew String(((*s)[i].getNumScholarships()-
											(*f)[0].getNumFs()).ToString());

			return i;  // return a copy of the currently displayed record's index
		}
		int fillFormFields(int p, char c)
		{
			static int i = 0;  // Keeps track of which record we are on
			i += p;		       // changes the current record
...
private: System::Void comboBoxType_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e) {
			 ...
			 case 0: updateData();
					 *iArrayUsed = 0;
					 fillFormFields(-int(s->size()+1));
					 break;

			 case 1: updateData();
				     *iArrayUsed = 1;
					 fillFormFields(-int(f->size()+1),'f');
					 break;
